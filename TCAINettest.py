...
parser.add_argument('--test_path',type=str,default='./VT5000/Test/',help='test dataset path')
...
#     num_workers=4,
#     pin_memory=True)
# for checkp in range(175, 175+1):
#     if checkp % 1 ==0:
    
#         with torch.no_grad():
#             modelpath= './BBSNet4_cpts/BBSNet_epoch_' + str(checkp)+ '.pth'
#             model.load_state_dict(torch.load(modelpath))
        
#             model.cuda()
#             model.eval()
#             save_path = './test_maps/VVT5000/'+str(checkp)+ '/'
...
#     if checkp % 1 ==0:
#         with torch.no_grad():
#             modelpath= './BBSNet3_cpts/BBSNet_epoch_' + str(checkp)+ '.pth'
#             model.load_state_dict(torch.load(modelpath))

#             model.cuda()
#             model.eval()
#             save_path = './test_maps/XVT1000/'+str(checkp)+ '/'
#             if not os.path.exists(save_path):
#                 os.makedirs(save_path)
#             loss_e= 0
...
for checkp in range(200, 200+1):
    if checkp % 1 ==0:
        with torch.no_grad():
            ...
            model.cuda()
            model.eval()
            save_path = './test_maps/XVT821/'+str(checkp)+ '/'
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            loss_e= 0
            for i, (image, gt_shape, depth, name) in enumerate(data_loader):
